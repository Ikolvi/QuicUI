...
public class FlutterLoader {
  ...
  public void ensureInitializationComplete(
    ...
    try (TraceSection e = TraceSection.scoped("FlutterLoader#ensureInitializationComplete")) {
      ...
      // ============================================================
      // QuicUI Code Push Integration - START (C++ JNI Integration)
      // ============================================================
      String quicuiPatchedAOTPath = null;
      try {
          Log.i(TAG, "QuicUI: Initializing code push loader...");
          
          // Create QuicUI loader (calls into C++ via JNI)
          QuicUICodePushLoader codePushLoader = new QuicUICodePushLoader(applicationContext);
          
          // Get patched AOT path from C++ (includes validation)
          quicuiPatchedAOTPath = codePushLoader.getPatchedAOTPath();
          
          if (quicuiPatchedAOTPath != null && !quicuiPatchedAOTPath.isEmpty()) {
              Log.i(TAG, "QuicUI: ✅ Found valid patch at: " + quicuiPatchedAOTPath);
              
              // Log patch info for debugging
              String patchInfo = codePushLoader.getPatchInfo();
              if (patchInfo != null) {
                  Log.i(TAG, "QuicUI: Patch details: " + patchInfo);
              }
          } else {
              Log.i(TAG, "QuicUI: No valid patch found, using original libapp.so");
          }
      } catch (Exception quicuiException) {
          Log.e(TAG, "QuicUI: Error in code push integration: " + quicuiException.getMessage());
          quicuiException.printStackTrace();
          // Continue with original libapp.so on error
          quicuiPatchedAOTPath = null;
      }
      // ============================================================
      ...
      if (BuildConfig.DEBUG || BuildConfig.JIT_RELEASE) {
        ...
        // QuicUI: Use patched path if available, otherwise use default
        String aotLibraryToUse = (quicuiPatchedAOTPath != null && !quicuiPatchedAOTPath.isEmpty())
            ? quicuiPatchedAOTPath
            : flutterApplicationInfo.aotSharedLibraryName;
        
        // Add AOT shared library name arg

...
#import "QuicUICodePushLoader.h"  // QuicUI Code Push
...
- (void)checkForCodePushPatches {
  @autoreleasepool {
    ...
    if (patchedPath) {
      ...
      // CRITICAL: Update settings to use patched AOT
      _settings.application_library_paths.clear();
      _settings.application_library_paths.push_back([patchedPath UTF8String]);
      NSLog(@"[QuicUI] ✅ Configured DartVM to load patched AOT snapshot");

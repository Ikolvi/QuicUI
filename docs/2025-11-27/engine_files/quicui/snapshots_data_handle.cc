...
// Note: The Dart_SnapshotDataSize and Dart_SnapshotInstrSize functions
// are not available in this version of Dart SDK. For the interpreter-based
// approach, we use size 0 which indicates unknown size. The actual size
// will be determined by the ELF loader.
static std::unique_ptr<fml::Mapping> DataMapping(const DartSnapshot& snapshot) {
  auto ptr = snapshot.GetDataMapping();
  return std::make_unique<fml::NonOwnedMapping>(ptr, 0 /* size unknown */);
}

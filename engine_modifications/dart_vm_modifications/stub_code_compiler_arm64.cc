...
// ============================================================================
// QuicUI Code Push Stubs
// ============================================================================
//
// These stubs handle transitions between native CPU execution and the ARM64
// simulator for iOS code push functionality.
// ============================================================================

#if defined(QUICUI_USE_INTERPRETER) && QUICUI_USE_INTERPRETER

// Stub that enters the interpreter (simulator) to run patched code.
void StubCodeCompiler::GenerateQuicuiResumeInterpreterStub() {
  ...
  // Push arguments for runtime call
  __ Push(ZR);   // Result slot
  __ Push(R0);   // Arg0: Target PC
  __ Push(R1);   // Arg1: Saved SP
  __ Push(THR);  // Arg2: Thread
  
  __ CallRuntime(kQuicuiResumeInterpreterRuntimeEntry, 3);
...
// Stub that exits simulator to call native code
void StubCodeCompiler::GenerateQuicuiResumeOnCPUStub() {
  __ mov(TMP, R0);
...
// Stub that returns to simulator after native call
void StubCodeCompiler::GenerateQuicuiResumeOnSimulatorStub() {
  __ EnterStubFrame();
  ...
  __ CallRuntime(kQuicuiResumeOnSimulatorRuntimeEntry, 2);
...
// Enter C frame from simulator
void StubCodeCompiler::GenerateQuicuiEnterCFrameStub() {
  __ PushPair(FP, LR);
  __ mov(FP, SP);
...
// Leave C frame
void StubCodeCompiler::GenerateQuicuiLeaveCFrameStub() {
  __ PopQuad(V7);
  __ PopQuad(V6);
  __ PopQuad(V5);
  __ PopQuad(V4);
...
// Dispatch to patched function via simulator
void StubCodeCompiler::GenerateQuicuiDispatchToPatchStub() {
  ...
  __ ldr(TMP, Address(THR, target::Thread::quicui_resume_interpreter_stub_offset()));
  __ blr(TMP);

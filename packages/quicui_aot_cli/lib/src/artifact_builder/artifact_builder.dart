...
/// Used to wrap code that invokes `flutter build` with Quicui's fork of
/// Flutter.
typedef QuicuiBuildCommand = Future<void> Function();
...
extension on String {
  ...
  ///   {'QUICUI_PUBLIC_KEY': this}
  ///
  /// QUICUI_PUBLIC_KEY is the name expected by the Quicui's Flutter tool
  ///
  /// This allow us to just call var?.toPublicKeyEnv() instead of doing
  /// a ternary operation to check if the value is null.
  Map<String, String> toPublicKeyEnv() => {'QUICUI_PUBLIC_KEY': this};
}
...
If the above command fails, then this is likely not a Quicui issue and
the underlying `flutter build` failure must be resolved for Quicui to
build a release.
...
Quicui will default to the latest stable version of Flutter.
We strongly encourage always specifying an explicit Flutter version:

${lightCyan.wrap('quicui release <platform> --flutter-version=3.29.0')}

• If `flutter build` completes successfully and `quicui release`
fails when using the same flutter version, please file an issue:
${link(uri: Uri.parse('https://github.com/quicuitech/quicui/issues/new'))}
''';
  ...
  Future<File> buildAppBundle({
    ...
    await _runQuicuiBuildCommand(() async {
...
Reason: Exited with code $exitCode.''',
    ...
    try {
      return quicuiAndroidArtifacts.findAab(
        project: projectRoot,
        flavor: flavor,
      );
  ...
  Future<File> buildApk({
    ...
    await _runQuicuiBuildCommand(() async {
      ...
      final arguments = [
        ...
        // See https://github.com/quicuitech/quicui/issues/1141.
...
Reason: Exited with code $exitCode.''',
    ...
    final projectRoot = quicuiEnv.getQuicuiProjectRoot()!;
    try {
      return quicuiAndroidArtifacts.findApk(
        project: projectRoot,
        flavor: flavor,
      );
    } on MultipleArtifactsFoundException catch (error) {
  ...
  Future<void> buildAar({
    ...
    return _runQuicuiBuildCommand(() async {
      const executable = 'flutter';
      final targetPlatformArgs = targetPlatforms?.targetPlatformArg;
      final arguments = [
        'build',
...
Reason: Exited with code $exitCode.''',
  ...
  /// --release` with Quicui's fork of Flutter.
...
Reason: Exited with code $exitCode.''',
    ...
    if (appDillPath == null) {
      throw ArtifactBuildException(
        ...
        fixRecommendation:
            '''Please file a bug at https://github.com/quicuitech/quicui/issues/new with the logs for this command.''',
      );
    }
...
Reason: Exited with code $exitCode.''',
  ...
  /// Quicui's fork of Flutter) with a try/finally that runs
  /// `flutter pub get` with the system installation of Flutter to reset
  /// `.dart_tool/package_config.json` to the system Flutter.
  Future<void> _runQuicuiBuildCommand(QuicuiBuildCommand command) async {
    try {
      await command();
    } finally {
  ...
  /// run by quicui update the package_config.json file to point to
  /// quicui's version of Flutter, which confuses VS Code. See
  /// https://github.com/quicuitech/quicui/issues/1101 for more info.
...
''');
  ...
  Future<File> buildElfAotSnapshot({
    ...
    required QuicuiArtifact genSnapshotArtifact,
    ...
    final exitCode = await process.stream(
      quicuiArtifacts.getArtifactPath(artifact: genSnapshotArtifact),
      arguments,
      // Never run in shell because we always have a fully resolved
      // executable path.
      runInShell: false,

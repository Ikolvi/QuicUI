...
/// Manages artifacts for the Quicui CLI.
class ArtifactManager {
  ...
  static Directory? androidArchsDirectory({
    ...
    //
    // See https://github.com/quicuitech/quicui/issues/1798
  ...
  /// Returns the path to the quicui release supplement directory for iOS.
  ///
  ...
  /// Returns the path to the quicui release supplement directory for macOS.
  ///
  /// Returns null if there is no supplement directory
  /// (e.g. when using older Flutter revisions).
  Directory? getMacosReleaseSupplementDirectory() {
    final projectRoot = quicuiEnv.getQuicuiProjectRoot()!;
    final releaseSupplementDir = Directory(
      p.join(projectRoot.path, 'build', 'macos', 'quicui'),
    );
  ...
  /// Name of the App.xcframework generated by `quicui release ios-framework`
  static const String appXcframeworkName = 'App.xcframework';

  /// Returns the path to the App.xcframework generated by
  /// `quicui release ios-framework` or
  /// `quicui patch ios-framework`.

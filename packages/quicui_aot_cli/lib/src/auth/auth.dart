...
import 'package:quicui_aot_cli/src/auth/ci_token.dart';
import 'package:quicui_aot_cli/src/auth/endpoints/endpoints.dart';
...
/// The environment variable that holds the Quicui CI token.
const quicuiTokenEnvVar = 'QUICUI_TOKEN';
  ...
  Future<oauth2.AccessCredentials> _tryRefreshCredentials(
    ...
    try {
      ...
      logger
        ...
        ..info(
          '''Try logging out with ${lightBlue.wrap('quicui logout')} and logging in again.''',
        )
        ..detail(e.toString())
        ..detail(s.toString());
...
class Auth {
  ...
  void _loadCredentials() {
    final envToken = platform.environment[quicuiTokenEnvVar];
    if (envToken != null) {
      logger.detail('[env] $quicuiTokenEnvVar detected');
...
Please regenerate using `quicui login:ci`, update the $quicuiTokenEnvVar environment variable, and try again.''')
          ..detail(e.toString());
        rethrow;
      }

      logger.detail('[env] $quicuiTokenEnvVar parsed');
      return;
    }
    ...
    switch (this) {
      case AuthProvider.google:
        return oauth2.ClientId(
          /// Quicui CLI's OAuth 2.0 identifier for GCP,
          '''523302233293-eia5antm0tgvek240t46orctktiabrek.apps.googleusercontent.com''',

          /// Quicui CLI's OAuth 2.0 secret for GCP.
          ///
      ...
      case AuthProvider.microsoft:
        return oauth2.ClientId(
          /// Quicui CLI's OAuth 2.0 identifier for Azure/Entra.

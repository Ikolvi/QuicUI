...
/// {@template cache}
/// A class that manages the artifacts cached by Quicui.
...
/// [QuicuiArtifacts] since uses the current Quicui environment.
/// {@endtemplate}
class Cache {
  /// {@macro cache}
  Cache() {
  ...
  Directory getArtifactDirectory(String name) {
    return Directory(
      p.join(quicuiArtifactsDirectory.path, p.withoutExtension(name)),
    );
  }
  ...
  Directory getPreviewDirectory(String name) {
    return Directory(
      p.join(quicuiPreviewsDirectory.path, p.withoutExtension(name)),
    );
  }

  /// The Quicui cache directory.
  static Directory get quicuiCacheDirectory {
    return Directory(p.join(quicuiEnv.quicuiRoot.path, 'bin', 'cache'));
  }

  /// The Quicui cached previews directory.
  static Directory get quicuiPreviewsDirectory {
    return Directory(p.join(quicuiCacheDirectory.path, 'previews'));
  }

  /// The Quicui cached artifacts directory.
  static Directory get quicuiArtifactsDirectory {
    return Directory(p.join(quicuiCacheDirectory.path, 'artifacts'));
  }
  ...
  String get storageBucket => 'download.quicui.dev';

  /// Clear the cache.
  Future<void> clear() async {
    final cacheDir = quicuiCacheDirectory;
...
/// An artifact which is cached by Quicui.
/// {@endtemplate}
abstract class CachedArtifact {
  ...
  /// Whether the artifact is required for Quicui to function.
...
If you're behind a firewall/proxy, please, make sure quicui_aot_cli is
...
class AotToolsArtifact extends CachedArtifact {
  ...
  @override
  String get storageUrl =>
      '${cache.storageBaseUrl}/${cache.storageBucket}/quicui/${quicuiEnv.quicuiEngineRevision}/$fileName';
...
class PatchArtifact extends CachedArtifact {
  ...
  String get storageUrl {
    ...
    return '${cache.storageBaseUrl}/${cache.storageBucket}/quicui/${quicuiEnv.quicuiEngineRevision}/$artifactName';
  }

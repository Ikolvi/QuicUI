...
import 'package:quicui_aot_cli/src/archive/directory_archive.dart';
...
class CodePushClientWrapper {
  ...
  Future<App> createApp({required int organizationId, String? appName}) async {
    ...
    if (appName == null) {
      final defaultAppName = quicuiEnv.getPubspecYaml()?.name;
...
This app may not exist or you may not have permission to view it.''');
  ...
  void ensureReleaseIsNotActive({
    ...
    if (release.platformStatuses[platform] == ReleaseStatus.active) {
      final uri = QuicuiWebConsole.appReleaseUri(release.appId, release.id);
      logger.err(
        '''
It looks like you have an existing ${platform.name} release for version ${lightCyan.wrap(release.version)}.
Please bump your version number and try again.

You can manage this release in the ${link(uri: uri, message: 'Quicui Console')}''',
      );
      throw ProcessExit(ExitCode.software.code);
    }
  }
...
Please create a release using "quicui release" and try again.
''');
      throw ProcessExit(ExitCode.software.code);
    }
...
Please run `quicui cache clean` and try again. If the issue persists, please
file a bug report at https://github.com/quicuitech/quicui/issues/new.
...
aar artifact already exists, continuing...''');
  ...
  /// Registers and uploads macOS release artifacts to the Quicui server.
  ...
  /// Publishes a patch to the Quicui server. This consists of creating a
  ...
  Never _handleErrorAndExit(
    ...
    if (error is CodePushUpgradeRequiredException) {
      progress?.fail();
      logger
        ..err('Your version of quicui is out of date.')
        ..info(
          '''Run ${lightCyan.wrap('quicui upgrade')} to get the latest version.''',
        );

...
/// Login as a CI user.
/// {@endtemplate}
class LoginCiCommand extends QuicuiCommand {
...
We could not find a Quicui account for ${error.email}.''')
        ..info(
          '''If you have not yet created an account, go to "${link(uri: Uri.parse('https://console.quicui.dev'))}" to create one. If you believe this is an error, please reach out to us via Discord, we're happy to help!''',
        );
...
${lightCyan.wrap('export $quicuiTokenEnvVar="\$QUICUI_TOKEN" && quicui patch android')}
''');

...
import 'package:quicui_aot_cli/src/archive_analysis/android_archive_differ.dart';
...
import 'package:quicui_aot_cli/src/commands/patch/patcher.dart';
...
class AndroidPatcher extends Patcher {
  ...
  /// https://github.com/quicuitech/updater/issues/211 was fixed in 3.24.2
...
See more info about the issue ${link(uri: Uri.parse('https://github.com/quicuitech/updater/issues/211'), message: 'on Github')}
''';
  ...
  Future<File> buildPatchArtifact({String? releaseVersion}) async {
    final flutterVersion = await quicuiFlutter.getVersion();
    // Android versions prior to 3.24.2 have a bug that can cause patches to
    // be erroneously uninstalled.
    // https://github.com/quicuitech/updater/issues/211 was fixed in 3.24.2
...
Looked in:
  ...
  Future<Map<Arch, PatchArtifactBundle>> createPatchArtifacts({
    ...
    // Direct users to https://github.com/quicuitech/quicui/issues/2532
...
Please refer to ${link(uri: Uri.parse('https://github.com/quicuitech/quicui/issues/2532'))} for potential workarounds.''',
        );
      }),
    );
  ...
  Future<String> extractReleaseVersionFromArtifact(File artifact) async {
    return quicuiAndroidArtifacts.extractReleaseVersionFromAppBundle(
      artifact.path,
    );
  }
}

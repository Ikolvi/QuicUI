...
class IosFrameworkPatcher extends Patcher {
  ...
  String get _aotOutputPath =>
      p.join(quicuiEnv.buildDirectory.path, 'out.aot');

  String get _vmcodeOutputPath =>
      p.join(quicuiEnv.buildDirectory.path, 'out.vmcode');

  String get _appDillCopyPath =>
      p.join(quicuiEnv.buildDirectory.path, 'app.dill');
  ...
  Future<Map<Arch, PatchArtifactBundle>> createPatchArtifacts({
    ...
    final aotSnapshotFile = File(
      p.join(quicuiEnv.getQuicuiProjectRoot()!.path, 'build', 'out.aot'),
    );
    // TODO(eseidel): Drop support for builds before the linker.
    final useLinker = AotTools.usesLinker(quicuiEnv.flutterRevision);
    if (useLinker) {
      apple.copySupplementFilesToSnapshotDirs(
        ...
        patchSupplementDir: quicuiEnv.iosSupplementDirectory,
        patchSnapshotDir: quicuiEnv.buildDirectory,
      );
    ...
    if (await aotTools.isGeneratePatchDiffBaseSupported()) {
      ...
      final analyzeSnapshotPath = quicuiArtifacts.getArtifactPath(

...
import 'package:quicui_aot_cli/src/commands/patch/patch.dart';
...
import 'package:quicui_aot_cli/src/formatters/formatters.dart';
...
/// A command that creates a quicui patch for the provided target platforms.
/// `quicui patch --platforms=android,ios`
/// {@endtemplate}
class PatchCommand extends QuicuiCommand {
  /// {@macro patch_command}
...
NOTE: this is ${styleBold.wrap('not')} recommended. Asset changes cannot be included in a patch can cause your app to behave unexpectedly.''';
  ...
  String get description =>
      'Creates a quicui patch for the provided target platforms';
  ...
  Future<void> createPatch(Patcher patcher) async {
    ...
    } else if (quicuiEnv.canAcceptUserInput) {
...
+-------------------------------------------------------------------------------+
''');
      lastBuiltFlutterRevision = quicuiEnv.flutterRevision;
    ...
    try {
      await quicuiFlutter.installRevision(revision: release.flutterRevision);
    } on Exception {
...
''');
        ...
        final baseMetadata = CreatePatchMetadata(
          ...
          environment: BuildEnvironmentMetadata(
            ...
            usesQuicuiCodePushPackage:
                quicuiEnv.usesQuicuiCodePushPackage,
          ),
        );
  ...
  void assertReleaseContainsPlatform({
    ...
    if (!contains) {
      ...
      logger.err(
        '''No release exists for $platformName in release version ${release.version}. Please run quicui release $platformName to create one.''',
      );
      throw ProcessExit(ExitCode.software.code);
    }
  }

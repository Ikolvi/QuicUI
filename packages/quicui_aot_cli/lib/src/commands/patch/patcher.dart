...
import 'package:quicui_aot_cli/src/extensions/iterable.dart';
...
abstract class Patcher {
  ...
  static String lowLinkPercentageWarning(double linkPercentage) {
    return '''
${lightCyan.wrap('quicui patch')} was only able to share ${linkPercentage.toStringAsFixed(1)}% of Dart code with the released app.
...
''';
  ...
  Directory get projectRoot => quicuiEnv.getQuicuiProjectRoot()!;
  ...
  static File get debugInfoFile {
    return File(p.join(quicuiEnv.buildDirectory.path, 'patch-debug.zip'));
  }
  ...
  ///
  /// Context: https://github.com/quicuitech/quicui/issues/2270

...
/// `quicui preview` command.
/// {@endtemplate}
class PreviewCommand extends QuicuiCommand {
...
This is only applicable when previewing Android releases.''',
  ...
  Future<int> run() async {
    ...
    final flavors = quicuiYaml?.flavors;

    if (results.wasParsed('app-id')) {
      appId = results['app-id'] as String;
    } else if (quicuiYaml != null && flavors == null) {
  ...
  Future<int> installAndLaunchMacos({
    ...
    // 2025-01-31 09:53:22.889 Df macos_sandbox[22535:1d268] [dev.quicui:updater::cache::patch_manager] [quicui] No public key provided, skipping signature verification
  ...
  /// Sets the channel property in the quicui.yaml file inside the Windows
  /// app whose root directory is [appDirectory].
  Future<void> setChannelOnWindowsApp({
    required Directory appDirectory,
    required String channel,
  }) async {
    final quicuiYamlFile = File(
      p.join(appDirectory.path, 'data', 'flutter_assets', 'quicui.yaml'),
    );

    await _maybeSetChannelInQuicuiYaml(
      channel: channel,
      quicuiYamlFile: quicuiYamlFile,
    );
  }

  /// Sets the channel property in the quicui.yaml file inside the Runner.app
  Future<void> setChannelOnRunner({
    required Directory runnerDirectory,
    required String channel,
  }) async {
    await Isolate.run(() async {
      final quicuiYaml = File(
        p.join(
      ...
      if (!quicuiYaml.existsSync()) {
        throw Exception('Unable to find quicui.yaml');
      }

      await _maybeSetChannelInQuicuiYaml(
        channel: channel,
        quicuiYamlFile: quicuiYaml,
      );
    });
  }

  /// Sets the channel property in the quicui.yaml file inside a macOS app.
  ...
  /// Unzips the `.aab` and sets the channel property in the quicui.yaml
  ...
  Future<void> setChannelOnAab({
    ...
    await Isolate.run(() async {
      ...
      final didUpdateQuicuiYaml = await _maybeSetChannelInQuicuiYaml(
        channel: channel,
        quicuiYamlFile: quicuiYamlFile,
      );

      if (!didUpdateQuicuiYaml) {
        return;
      }
  ...
  /// Sets the channel property in the quicui.yaml file if none is set.
  Future<void> setChannelOnLinuxApp({
    required String channel,
    required Directory bundleDirectory,
  }) async {
    final quicuiYamlFile = File(
      p.join(bundleDirectory.path, 'data', 'flutter_assets', 'quicui.yaml'),
    );
  ...
  /// Sets the channel property in the quicui.yaml file if none is set or
  /// different from the provided channel.
  static Future<bool> _maybeSetChannelInQuicuiYaml({
    required String channel,
    required File quicuiYamlFile,
  }) async {
    if (!quicuiYamlFile.existsSync()) {
      throw Exception('Unable to find quicui.yaml');
    }

    final yamlText = quicuiYamlFile.readAsStringSync();
    ...
    quicuiYamlFile.writeAsStringSync(yamlEditor.toString(), flush: true);
    return true;
  }
}

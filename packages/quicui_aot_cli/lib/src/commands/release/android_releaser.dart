...
To change the version of this release, change your app's version in your pubspec.yaml.''',
    ...
    if (generateApk && splitApk) {
      logger
        ..err(
          'Quicui does not support the split-per-abi option at this time',
        )
        ..info(
          '''
...
Please comment and upvote ${link(uri: Uri.parse('https://github.com/quicuitech/quicui/issues/1141'))} if you would like quicui to support this.''',
        );
  ...
  Future<String> getReleaseVersion({
    ...
    try {
      releaseVersion = await quicuiAndroidArtifacts
  ...
  Future<void> uploadReleaseArtifacts({
    ...
    final aabFile = quicuiAndroidArtifacts.findAab(
      project: projectRoot,
      flavor: flavor,
    );
  ...
  String get postReleaseInstructions {
    ...
    if (generateApk) {
      final apkFile = quicuiAndroidArtifacts.findApk(
        project: projectRoot,
        flavor: flavor,
      );
      apkText = generateApk

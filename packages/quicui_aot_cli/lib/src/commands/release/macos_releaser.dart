...
import 'package:quicui_aot_cli/src/archive_analysis/plist.dart';
...
import 'package:quicui_aot_cli/src/metadata/update_release_metadata.dart';
...
To change the version of this release, change your app's version in your pubspec.yaml.''',
    ...
    if (argResults.rest.contains('--obfuscate')) {
      ...
      // See https://github.com/quicuitech/quicui/issues/1619
      logger
        ..err('Quicui does not currently support obfuscation on macOS.')
        ..info(
          '''We hope to support obfuscation in the future. We are tracking this work at ${link(uri: Uri.parse('https://github.com/quicuitech/quicui/issues/1619'))}.''',
        );
  ...
  Future<FileSystemEntity> buildReleaseArtifacts() async {
    if (!codesign) {
      logger
        ...
        ..warn(
          '''quicui preview will not work for releases created with "--no-codesign". However, you can still preview your app by signing the generated .xcarchive in Xcode.''',
        );
    }
  ...
  Future<void> uploadReleaseArtifacts({
    ...
    if (quicuiEnv.macosPodfileLockFile.existsSync()) {
      podfileLockHash = sha256
          .convert(quicuiEnv.macosPodfileLockFile.readAsBytesSync())
          .toString();
    } else {

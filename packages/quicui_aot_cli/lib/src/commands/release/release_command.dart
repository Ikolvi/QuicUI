...
import 'package:quicui_aot_cli/src/commands/release/release.dart';
...
import 'package:quicui_aot_cli/src/extensions/string.dart';
...
/// {@endtemplate}
class ReleaseCommand extends QuicuiCommand {
...
of the iOS app that is using this module. (aar and ios-framework only)''',
  ...
  String get description =>
      'Creates a quicui release for the provided target platforms';
  ...
  /// The quicui app ID for the current project.
  String get appId => quicuiEnv.getQuicuiYaml()!.getAppId(flavor: flavor);
  ...
  /// The workflow to create a new release for a Quicui app.
  ///
  ...
  Future<void> createRelease(Releaser releaser) async {
    ...
    try {
      await quicuiValidator.validateFlavors(
        flavorArg: flavor,
    ...
    try {
      await quicuiFlutter.installRevision(revision: targetFlutterRevision);
    } on Exception {
    ...
    final releaseFlutterQuicuiEnv = quicuiEnv.copyWith(
    ...
    return await runScoped(
      () async {
        ...
        final flutterVersionString = await quicuiFlutter
...
✅ Published Release ${release.version}!''')
      ...
      values: {quicuiEnvRef.overrideWith(() => releaseFlutterQuicuiEnv)},
    );
  }
  ...
  Future<void> assertArgsAreValid(Releaser releaser) async {
    ...
    final version = await quicuiFlutter.resolveFlutterVersion(
      flutterVersionArg,
    );
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}''');
  ...
  /// [quicuiEnv]. Will exit with [ExitCode.software] if the version
  /// specified by the user is not found/supported.
  Future<String> resolveTargetFlutterRevision() async {
    if (flutterVersionArg == 'latest') return quicuiEnv.flutterRevision;

    final String? revision;
    try {
      revision = await quicuiFlutter.resolveFlutterRevision(
        flutterVersionArg,
      );
...
$error''');
    ...
    if (revision == null) {
      final openIssueLink = link(
        uri: Uri.parse(
          'https://github.com/quicuitech/quicui/issues/new?assignees=&labels=feature&projects=&template=feature_request.md&title=feat%3A+',
        ),
        message: 'open an issue',
      );
      logger.err('''
Version $flutterVersionArg not found. Please $openIssueLink to request a new version.
Use `quicui flutter versions list` to list available versions.
''');
      throw ProcessExit(ExitCode.software.code);
    }
  ...
  Future<void> ensureVersionIsReleasable({
    ...
    if (existingRelease != null) {
      ...
      if (existingRelease.flutterRevision != flutterRevision) {
        final flutterVersion = await quicuiFlutter.getVersionForRevision(
          flutterRevision: flutterRevision,
        );

        final formattedCurrentReleaseVersion = quicuiFlutter.formatVersion(
          revision: flutterRevision,
          version: flutterVersion,
        );

        final formattedExistingReleaseVersion = quicuiFlutter.formatVersion(
...
To resolve this issue, you can:
  ...
  * Bump the release version and re-run the release command with the desired Flutter version.''',
  ...
  Future<void> confirmCreateRelease({
    ...
    final flutterVersionString = await quicuiFlutter.getVersionAndRevision();
...
''');
    ...
    if (quicuiEnv.canAcceptUserInput && !noConfirm) {
  ...
  Future<Release> getOrCreateRelease({
    ...
    return await codePushClientWrapper.maybeGetRelease(
        ...
        await codePushClientWrapper.createRelease(
          appId: appId,
          version: version,
          flutterRevision: quicuiEnv.flutterRevision,
          platform: releasePlatform,
        );
  }
  ...
  Future<void> finalizeRelease({
    ...
    final baseMetadata = UpdateReleaseMetadata(
      ...
      environment: BuildEnvironmentMetadata(
        ...
        quicuiYaml: quicuiEnv.getQuicuiYaml()!,
        usesQuicuiCodePushPackage: quicuiEnv.usesQuicuiCodePushPackage,
      ),
    );

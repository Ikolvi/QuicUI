...
/// {@endtemplate}
class Doctor {
  /// Validators that verify quicui will work on Android.
  final List<Validator> androidCommandValidators = [
    AndroidInternetPermissionValidator(),
  ];

  /// Validators that verify quicui will work on iOS.
  final List<Validator> iosCommandValidators = [];

  /// Validators that verify quicui will work on Linux.
  final List<Validator> linuxCommandValidators = [];

  /// Validators that verify quicui will work on macOS.
  final List<Validator> macosCommandValidators = [MacosEntitlementsValidator()];

  /// Validators that verify quicui will work on Windows.
...
No issues detected!''');
      ...
      if (allFixableIssues.isNotEmpty && !applyFixes) {
        ...
        logger.info(
          '''
$fixableIssueCount issue${fixableIssueCount == 1 ? '' : 's'} can be fixed automatically with ${lightCyan.wrap('quicui doctor --fix')}.''',
        );
      }
    }
  }

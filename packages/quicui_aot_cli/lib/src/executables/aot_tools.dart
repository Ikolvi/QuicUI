...
/// Wrapper around the quicui `aot-tools` executable.
class AotTools {
  ...
  static bool usesLinker(String flutterRevision) {
    ...
    // This will be wrong if we ever need new quicui to work with old local
    // engine build.
  ...
  Future<QuicuiProcessResult> _exec(
    List<String> command, {
    ...
    final artifactPath = quicuiArtifacts.getArtifactPath(
      artifact: QuicuiArtifact.aotTools,
    );

    final QuicuiProcessResult result;

    // Similar to [QuicuiProcess.run] but uses [QuicuiProcess.start]
    // instead and captures the live stdout and stderr of the process.
    Future<QuicuiProcessResult> execute(
      String exe,
      List<String> args, {
      String? workingDirectory,
    }) async {
    ...
    // Fallback behavior for older versions of quicui where aot-tools was
    ...
    if (extension != '.dill' && extension != '.dart') {
      ...
      result = await execute(quicuiEnv.dartBinaryFile.path, [
        'run',
        artifactPath,
        ...command,
      ], workingDirectory: workingDirectory);

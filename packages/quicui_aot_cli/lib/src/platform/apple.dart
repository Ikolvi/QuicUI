...
class Apple {
  ...
  // `aot_tools` rather than having to know the contents within `quicui`.
  void copySupplementFilesToSnapshotDirs({
    ...
    // use the same supplement files. At the `quicui` level we don't know
  ...
  Set<String>? flavors({required ApplePlatform platform}) {
    ...
    // https://github.com/quicuitech/quicui/issues/1703 for an example.
  ...
  Future<LinkResult> runLinker({
    ...
    final buildDirectory = quicuiEnv.buildDirectory;
    ...
    final genSnapshot = quicuiArtifacts.getArtifactPath(

...
import 'package:quicui_aot_cli/src/config/quicui_yaml.dart';
...
/// A reference to a [QuicuiEnv] instance.
final quicuiEnvRef = create(QuicuiEnv.new);

/// The [QuicuiEnv] instance available in the current zone.
QuicuiEnv get quicuiEnv => read(quicuiEnvRef);

/// {@template quicui_env}
/// A class that provides access to quicui environment metadata.
/// {@endtemplate}
class QuicuiEnv {
  /// {@macro quicui_env}
  const QuicuiEnv({
  ...
  /// Copy the [QuicuiEnv] and optionally override the flutter revision.
  QuicuiEnv copyWith({String? flutterRevisionOverride}) => QuicuiEnv(
  ...
  /// The application config directory for the Quicui CLI.
  Directory get configDirectory {
    return Directory(applicationConfigHome(executableName));
  }

  /// The directory where quicui logs are stored.
  Directory get logsDirectory {
    return Directory(p.join(configDirectory.path, 'logs'));
  }

  /// The root directory of the Quicui install.
  ///
  /// Assumes we are running from $ROOT/bin/cache.
  Directory get quicuiRoot {
    return File(platform.script.toFilePath()).parent.parent.parent;
  }

  /// The Quicui engine revision.
  String get quicuiEngineRevision {
    return File(
      p.join(flutterDirectory.path, 'bin', 'internal', 'engine.version'),
    ).readAsStringSync().trim();
  }

  /// Get the Quicui Flutter revision.
  String get flutterRevision {
    return _flutterRevisionOverride ??
        File(
          p.join(quicuiRoot.path, 'bin', 'internal', 'flutter.version'),
        ).readAsStringSync().trim();
  }

  /// Whether the project uses package:quicui_code_push.
  bool get usesQuicuiCodePushPackage {
    final pubspec = getPubspecYaml();
    return pubspec?.dependencies.containsKey('quicui_code_push') ?? false;
  }

  /// The root of the Quicui-vended Flutter git checkout.
  Directory get flutterDirectory {
    return Directory(
      p.join(quicuiRoot.path, 'bin', 'cache', 'flutter', flutterRevision),
    );
  }

  /// The Quicui-vended Flutter binary.
  ...
  /// The build directory of the current quicui project.
  ...
  Directory get iosSupplementDirectory =>
      Directory(p.join(buildDirectory.path, 'ios', 'quicui'));

  /// The `quicui.yaml` file for this project.
  File getQuicuiYamlFile({required Directory cwd}) {
    return File(p.join(cwd.path, 'quicui.yaml'));
  }
  ...
  /// Returns the root directory of the nearest Quicui project.
  Directory? getQuicuiProjectRoot() {
    final file = findNearestAncestor(
      where: (path) => getQuicuiYamlFile(cwd: Directory(path)),
    );
  ...
  /// The `quicui.yaml` file for this project, parsed into a [QuicuiYaml]
  /// object.
  ///
  /// Returns `null` if the file does not exist.
  /// Throws a [ParsedYamlException] if the file exists but is invalid.
  QuicuiYaml? getQuicuiYaml() {
    final root = getQuicuiProjectRoot();
    if (root == null) return null;
    final yaml = getQuicuiYamlFile(cwd: root).readAsStringSync();
    return checkedYamlDecode(yaml, (m) => QuicuiYaml.fromJson(m!));
  }
  ...
  /// Whether the current project has a `quicui.yaml` file.
  bool get hasQuicuiYaml => getQuicuiYaml() != null;
  ...
  /// `quicui.yaml` in its `assets` section.
  bool get pubspecContainsQuicuiYaml {
    final pubspec = getPubspecYaml();
    ...
    return assets.contains('quicui.yaml');
  }
  ...
  /// The base URL for the Quicui code push server that overrides the default

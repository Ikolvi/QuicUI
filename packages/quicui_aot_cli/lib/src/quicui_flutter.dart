...
import 'package:quicui_aot_cli/src/extensions/version.dart';
...
/// A reference to a [QuicuiFlutter] instance.
final quicuiFlutterRef = create(QuicuiFlutter.new);

/// The [QuicuiFlutter] instance available in the current zone.
QuicuiFlutter get quicuiFlutter => read(quicuiFlutterRef);

/// {@template quicui_flutter}
/// Helps manage the Flutter installation used by Quicui.
/// {@endtemplate}
class QuicuiFlutter {
  /// {@macro quicui_flutter}
  const QuicuiFlutter();

  /// The executable name.
  static const executable = 'flutter';

  /// The Quicui Flutter fork git URL.
  static const String flutterGitUrl =
      'https://github.com/quicuitech/flutter.git';
  ...
  String _workingDirectory({String? revision}) {
    revision ??= quicuiEnv.flutterRevision;
    return p.join(quicuiEnv.flutterDirectory.parent.path, revision);
  }
  ...
  Future<void> installRevision({required String revision}) async {
    ...
    try {
      // Clone the Quicui Flutter repo into the target directory.
      await git.clone(
        url: flutterGitUrl,
  ...
  /// Returns the current Quicui Flutter version and revision.
  ...
  /// Returns the current Quicui Flutter version.
  /// Throws a [ProcessException] if the version check fails.
  /// Returns `null` if the version check succeeds but the version cannot be
  /// parsed.
  Future<String?> getVersionString() async {
    final flutterRevision = quicuiEnv.flutterRevision;
    return getVersionForRevision(flutterRevision: flutterRevision);
  }

  /// The current Quicui Flutter version as a [Version]. Returns null if the

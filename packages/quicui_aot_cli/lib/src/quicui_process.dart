...
/// A reference to a [QuicuiProcess] instance.
final processRef = create(QuicuiProcess.new);

/// The [QuicuiProcess] instance available in the current zone.
QuicuiProcess get process => read(processRef);

/// A wrapper around [Process] that replaces executables to Quicui-vended
/// versions.
// This may need a better name, since it returns "Process" it's more a
// "ProcessFactory" than a "Process".
class QuicuiProcess {
  /// Creates a QuicuiProcess.
  QuicuiProcess({
    ProcessWrapper? processWrapper, // For mocking QuicuiProcess.
  ...
  Future<QuicuiProcessResult> run(
    String executable,
    List<String> arguments, {
    Map<String, String>? environment,
    bool? runInShell,
  ...
  String _resolveExecutable(
    ...
    if (useVendedFlutter && executable == 'flutter') {
      return _sanitizeExecutablePath(quicuiEnv.flutterBinaryFile.path);
    }
  ...
  List<String> _resolveArguments(
    ...
    if (executable == 'flutter') {
      if (logger.level == Level.verbose) {
        ...
        /// quicui command was run with `--verbose`
        /// (e.g. `quicui release ios --verbose`).
  ...
  void _logResult(QuicuiProcessResult result) {
...
$stderr''');
  ...
  Map<String, String> _environmentOverrides({required String executable}) {
    if (executable == 'flutter') {
      // If this ever changes we also need to update the `quicui` shell
      // wrapper which downloads runs Flutter to fetch artifacts the first time.
      return {'FLUTTER_STORAGE_BASE_URL': 'https://download.quicui.dev'};
    }
...
class QuicuiProcessResult {
  /// Creates a new [QuicuiProcessResult].
  const QuicuiProcessResult({

...
void main() {
  group(ArtifactBuilder, () {
    ...
    late QuicuiAndroidArtifacts quicuiAndroidArtifacts;
    late QuicuiArtifacts quicuiArtifacts;
    late QuicuiEnv quicuiEnv;
    late QuicuiProcess quicuiProcess;
    late QuicuiProcessResult pubGetProcessResult;
    ...
    R runWithOverrides<R>(R Function() body) {
      return runScoped(
        ...
        values: {
          ...
          quicuiAndroidArtifactsRef.overrideWith(
            () => quicuiAndroidArtifacts,
          ),
        },
      );
    }
    ...
    setUpAll(() {
      registerFallbackValue(FakeQuicuiProcess());
    ...
    setUp(() {
      ...
      quicuiAndroidArtifacts = MockQuicuiAndroidArtifacts();
      ...
      when(() => quicuiEnv.flutterRevision).thenReturn('1234');

      when(quicuiEnv.getQuicuiProjectRoot).thenReturn(projectRoot);
...
'''),
    ...
    group('buildAppBundle', () {
      setUp(() {
        when(
          () => quicuiAndroidArtifacts.findAab(
        ...
        setUp(() {
          when(
            () => quicuiProcess.stream(
              ...
              environment: {'QUICUI_PUBLIC_KEY': base64PublicKey},
              runInShell: false,
            ),
          ).thenAnswer((_) async => ExitCode.success.code);
        });

        test('adds the QUICUI_PUBLIC_KEY to the environment', () async {
          await runWithOverrides(
            () => builder.buildAppBundle(
              flavor: 'flavor',
              target: 'target',
    ...
    group('buildApk', () {
      setUp(() {
        when(
          () => quicuiAndroidArtifacts.findApk(
            project: any(named: 'project'),
            flavor: any(named: 'flavor'),
          ),
...
Reason: Exited with code 70.'''),
        ...
        test('provides public key as environment variable', () async {
          ...
          verify(
            () => quicuiProcess.stream(
              'flutter',
              ['build', 'linux', '--release'],
              environment: {'QUICUI_PUBLIC_KEY': publicKey},
              runInShell: false,
            ),
          ).called(1);
        });
    ...
    group('buildIosFramework', () {
      ...
      group('buildElfAotSnapshot', () {
        setUp(() {
          when(
            () => quicuiArtifacts.getArtifactPath(
        ...
        test('passes additional args to gen_snapshot', () async {
          await runWithOverrides(
            () => builder.buildElfAotSnapshot(
              ...
              genSnapshotArtifact: QuicuiArtifact.genSnapshotIos,
              additionalArgs: ['--foo', 'bar'],
            ),
          );

          verify(
            () => quicuiProcess.stream('gen_snapshot', [
              '--deterministic',

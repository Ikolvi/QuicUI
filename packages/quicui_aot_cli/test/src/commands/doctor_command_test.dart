...
void main() {
  group('doctor', () {
    ...
    const quicuiFlutterRevision = 'test-flutter-revision';
    ...
    setUp(() {
      ...
      ).thenReturn(quicuiFlutterRevision);
      ...
      when(quicuiFlutter.getVersionString).thenAnswer((_) async => null);

      command = runWithOverrides(DoctorCommand.new)
        ..testArgResults = argResults;
    });

    test('prints quicui version, flutter revision, '
        'and engine revision '
...
Quicui v$packageVersion • git@github.com:quicuitech/quicui.git
Flutter • revision ${quicuiEnv.flutterRevision}
...
'''),
    ...
    test(
      '''prints quicui version, flutter revision, flutter version, and engine revision''',
      () async {
...
Quicui v$packageVersion • git@github.com:quicuitech/quicui.git
Flutter $flutterVersion • revision ${quicuiEnv.flutterRevision}
Engine • revision $quicuiEngineRevision
'''),
        ).called(1);

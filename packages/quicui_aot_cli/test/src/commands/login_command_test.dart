...
import 'package:quicui_aot_cli/src/commands/login_command.dart';
...
void main() {
  group(LoginCommand, () {
    ...
    test('exits with code 70 if no user is found', () async {
      ...
      verify(
        () => logger.err('We could not find a Quicui account for $email.'),
      ).called(1);
      verify(
        () => logger.info(any(that: contains('console.quicui.dev'))),
      ).called(1);
    });

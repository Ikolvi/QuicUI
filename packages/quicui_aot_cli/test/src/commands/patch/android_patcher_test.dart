...
void main() {
  group(AndroidPatcher, () {
    ...
    group('buildPatchArtifact', () {
      ...
      // See https://github.com/quicuitech/updater/issues/211
...
Looked in:
  ...
  - build/app/intermediates/stripped_native_libs/{flavor}Release/out/lib'''),
          ...
          test('prints message directing users to github issue', () async {
            ...
            verify(
              () => logger.info(
                any(
                  that: contains(
                    'https://github.com/quicuitech/quicui/issues/2532',
                  ),
                ),
              ),
            ).called(1);
    ...
    group('extractReleaseVersionFromArtifact', () {
      setUp(() {
        when(
          () => quicuiAndroidArtifacts.extractReleaseVersionFromAppBundle(
            any(),
          ),
      ...
      test(
        '''returns value of quicuiAndroidArtifacts.extractReleaseVersionFromAppBundle''',
        () async {
          expect(
            await runWithOverrides(

...
import 'package:quicui_aot_cli/src/archive_analysis/apple_archive_differ.dart';
...
import 'package:quicui_aot_cli/src/executables/aot_tools.dart';
...
void main() {
  group(IosPatcher, () {
    ...
    setUpAll(() {
      ...
      registerFallbackValue(QuicuiArtifact.genSnapshotIos);
    ...
    setUp(() {
      ...
      when(
        () => quicuiEnv.buildDirectory,
      ).thenReturn(Directory(p.join(projectRoot.path, 'build')));
      when(() => quicuiEnv.iosSupplementDirectory).thenReturn(
        Directory(p.join(projectRoot.path, 'build', 'quicui', 'ios')),
      );
      when(() => quicuiEnv.iosPodfileLockHash).thenReturn(null);
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}'''),
    ...
    group('createPatchArtifacts', () {
      ...
      void setUpProjectRootArtifacts() {
        ...
        File(
          p.join(projectRoot.path, 'build', 'ios', 'quicui', 'App.ct.link'),
        ).createSync(recursive: true);
        File(
          p.join(
            projectRoot.path,
      ...
      group('when uses linker', () {
        ...
        setUp(() {
          final quicuiRoot = Directory.systemTemp.createTempSync();
          flutterDirectory = Directory(
            p.join(quicuiRoot.path, 'bin', 'cache', 'flutter'),
          );
...
<dict>
	...
	<dict>
		...
		<string>com.quicui.timeShift</string>

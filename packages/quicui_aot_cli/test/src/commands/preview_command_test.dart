...
import 'package:quicui_aot_cli/src/deployment_track.dart';
import 'package:quicui_aot_cli/src/executables/devicectl/apple_device.dart';
...
void main() {
  group(PreviewCommand, () {
    ...
    Future<void> setupAndroidQuicuiYaml(Invocation invocation) async {
      File(
          p.join(
    ...
    Future<File> quicuiYamlFileFromAab(File aab) async {
      final tempDir = Directory.systemTemp.createTempSync();
      final aabDirectory = Directory(p.join(tempDir.path, 'app-release'))
        ..createSync(recursive: true);
    ...
    setUp(() {
      ...
      when(() => quicuiEnv.getQuicuiYaml()).thenReturn(null);
      when(
      ...
      group(
        ...
        () {
          setUp(() {
            ...
            ).thenAnswer(setupAndroidQuicuiYaml);
      ...
      group('setChannelOnAab', () {
        ...
        group('when channel is not set', () {
          group('when target channel is  production', () {
            test('does not change quicui.yaml', () async {
              ...
              final updatedQuicuiYamlFile = await quicuiYamlFileFromAab(
                aabFile,
              );
              expect(
                updatedQuicuiYamlFile.readAsStringSync(),
                'app_id: $appId\n',
              );
            });
          });
          ...
          group('when target channel is not production', () {
            test('sets quicui.yaml channel to target channel', () async {
              aabFile = await createAabFile(channel: null);
              await runWithOverrides(
                () =>
              ...
              expect(updatedQuicuiYamlFile.readAsStringSync(), '''
app_id: $appId
channel: live
''');
            });
...
''');
      ...
      group('when unable to find quicui.yaml', () {
        test('exits with code 70', () async {
          final result = await runWithOverrides(command.run);
          expect(result, equals(ExitCode.software.code));
          verify(
            () => progress.fail('Exception: Unable to find quicui.yaml'),
          ).called(1);
        });
      });
      ...
      group('when in a quicui project without flavors', () {
        setUp(() {
          when(
            () => quicuiEnv.getQuicuiYaml(),
          ).thenReturn(const QuicuiYaml(appId: 'test-app-id'));
      ...
      group('when in quicui project with flavors', () {
        setUp(() {
          when(() => quicuiEnv.getQuicuiYaml()).thenReturn(
            const QuicuiYaml(
              appId: 'test-app-id',
    ...
    group('ios', () {
      ...
      group('when install/launch succeeds (production)', () {
        late File quicuiYaml;
        setUp(() {
          quicuiYaml = setupIOSQuicuiYaml();
          when(
            () => iosDeploy.installAndLaunchApp(
              bundlePath: any(named: 'bundlePath'),
              deviceId: any(named: 'deviceId'),
        ...
        test('exits with code 0', () async {
          ...
          expect(quicuiYaml.readAsStringSync(), equals('app_id: $appId'));
        });
      });
...
'''),
    ...
    group('linux', () {
      ...
      setUp(() {
        ...
        when(
          () => quicuiProcess.start(any(), any()),
        ).thenAnswer((_) async => process);
        when(() => quicuiProcess.run(any(), any())).thenAnswer(
          (_) async => const QuicuiProcessResult(
            exitCode: 0,
            stdout: null,
            stderr: null,
          ),
      ...
      group('when preview artifact is not cached', () {
        ...
        test('downloads and launches artifact', () async {
          ...
          verify(
            () =>
                quicuiProcess.start(any(that: endsWith(executableName)), []),
          ).called(1);
      ...
      group('when preview artifact is cached', () {
        ...
        test('launches cached artifact', () async {
          ...
          verify(() => quicuiProcess.run('chmod', any())).called(1);
          verify(
            () =>
                quicuiProcess.start(any(that: endsWith(executableName)), []),
          ).called(1);
    ...
    group('macos', () {
      ...
      group('staging', () {
        ...
        setUp(() {
          ...
          quicuiYaml = setupMacosQuicuiYaml();
        });

        test('sets channel in quicui.yaml', () async {
          final result = await runWithOverrides(command.run);
          expect(result, equals(ExitCode.success.code));
...
2025-01-31 10:15:29.808 Df macos_sandbox[33557:2cc12] [dev.quicui:updater::updater] [quicui] Reporting successful launch.
'''
                .split('\n');

        setUp(() {
          setupMacosQuicuiYaml();
        ...
        test('removes noisy lines from log output', () async {
          ...
          verify(
            () => logger.info(
              '''[dev.quicui:updater::updater] [quicui] Reporting successful launch.''',
            ),
          ).called(1);
          verifyNever(
            () => logger.info(
    ...
    group('windows', () {
      ...
      void createQuicuiYaml({String? channel}) {
        final quicuiYaml =
            File(
                p.join(
      ...
      group('when preview artifact is not cached', () {
        ...
        test('downloads and launches artifact', () async {
          ...
          verify(
            () => quicuiProcess.start(any(that: endsWith('runner.exe')), []),
          ).called(1);
          verify(() => logger.info('hello world')).called(1);
          verify(() => logger.err('hello error')).called(1);
        });

...
import 'package:quicui_aot_cli/src/commands/release/aar_releaser.dart';
...
void main() {
  group(AarReleaser, () {
    ...
    setUp(() {
      ...
      when(() => quicuiEnv.androidPackageName).thenReturn(packageName);
    ...
    group('minimumFlutterVersion', () {
      test('is null', () {
        // Quicui has always had aar support, so we don't need to
    ...
    group('assertPreconditions', () {
      group('when validation succeeds', () {
        ...
        group('when androidPackageName is null', () {
          setUp(() {
            when(() => quicuiEnv.androidPackageName).thenReturn(null);
          });
    ...
    group('uploadReleaseArtifacts', () {
      ...
      setUp(() {
        when(
          () => quicuiAndroidArtifacts.extractAar(
...
1. Add the aar repo and Quicui's maven url to your app's settings.gradle:
...
dependencyResolutionManagement {
    ...
    repositories {
        ...
        mavenCentral()
+       maven {
+           url '../${p.basename(quicuiEnv.getQuicuiProjectRoot()!.path)}/${p.relative(p.join(projectRoot.path, 'release'))}'
+       }
+       maven {
-           url 'https://storage.googleapis.com/download.flutter.io'
+           url 'https://download.quicui.dev/download.flutter.io'
+       }
    }
}
''')}
...
dependencies {
  // ...
  releaseImplementation '${quicuiEnv.androidPackageName}:flutter_release:$buildNumber'
  // ...
}''')}
'''),
        );

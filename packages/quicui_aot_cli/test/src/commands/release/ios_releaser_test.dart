...
import 'package:quicui_aot_cli/src/commands/release/ios_releaser.dart';
...
To change the version of this release, change your app's version in your pubspec.yaml.''',
      ...
      group('when --obfuscate is passed', () {
        ...
        test('logs error and exits', () async {
          ...
          verify(
            () => logger.err(
              'Quicui does not currently support obfuscation on iOS.',
            ),
          ).called(1);
          verify(
            () => logger.info(
...
<dict>
    ...
    group('uploadReleaseArtifacts', () {
      ...
      setUp(() {
        ...
        when(() => quicuiEnv.iosPodfileLockFile).thenReturn(podfileLockFile);
      });
    ...
    group('updatedReleaseMetadata', () {
      ...
      const metadata = UpdateReleaseMetadata(
        ...
        environment: BuildEnvironmentMetadata(
          ...
          usesQuicuiCodePushPackage: true,
        ),
      );

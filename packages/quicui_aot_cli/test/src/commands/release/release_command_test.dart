...
import 'package:quicui_aot_cli/src/third_party/flutter_tools/lib/src/base/process.dart';
...
void main() {
  group(ReleaseCommand, () {
    ...
    const quicuiYaml = QuicuiYaml(appId: appId);
    ...
    setUp(() {
      ...
      when(
        ...
        when(() => quicuiEnv.flutterRevision).thenReturn(
          invocation.namedArguments[#flutterRevisionOverride] as String,
        );
        return quicuiEnv;
      });
      when(() => quicuiEnv.quicuiRoot).thenReturn(quicuiRoot);
      when(
      ...
      when(() => quicuiEnv.usesQuicuiCodePushPackage).thenReturn(true);
      ...
      when(
        () =>
            quicuiFlutter.installRevision(revision: any(named: 'revision')),
      ).thenAnswer((_) async => {});
      when(
...
✅ Published Release ${release.version}!'''),
        ...
        () => logger.info(
          '''To create a patch for this release, run ${lightCyan.wrap('quicui patch --platforms=android --release-version=${release.version}')}''',
        ),
      ]);
    });
...
✅ Published Release ${release.version}!'''),
          () => logger.info(postReleaseInstructions),
          () => logger.info(
            '''To create a patch for this release, run ${lightCyan.wrap('quicui patch --platforms=android --flavor=$flavor --target=$target --release-version=${release.version}')}''',
          ),
        ]);
      });
    ...
    group('when there is an existing release with the same version', () {
      ...
      group('when the release uses a different version of Flutter', () {
        setUp(() {
          ...
          when(
            () => quicuiFlutter.getVersionForRevision(
              flutterRevision: flutterRevision,
            ),
    ...
    group('when flutter-version is provided', () {
      ...
      group('when unable to determine flutter revision', () {
        ...
        setUp(() {
          when(
            () => quicuiFlutter.resolveFlutterRevision(any()),
          ).thenThrow(exception);
        });
      ...
      group('when flutter version is supported', () {
        ...
        test('uses specified flutter version to build '
          ...
          when(() => releaser.buildReleaseArtifacts()).thenAnswer((_) async {
            ...
            expect(quicuiEnv.flutterRevision, equals(revision));
            return File('');
          });

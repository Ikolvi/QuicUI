...
void main() {
  group(QuicuiFlutter, () {
    ...
    late QuicuiProcessResult versionProcessResult;
    late QuicuiProcessResult precacheProcessResult;
    ...
    setUp(() {
      ...
      flutterDirectory = Directory(p.join(quicuiRoot.path, 'flutter'));
      git = MockGit();
      ...
      versionProcessResult = MockQuicuiProcessResult();
      precacheProcessResult = MockQuicuiProcessResult();
      quicuiFlutter = runWithOverrides(QuicuiFlutter.new);
    ...
    group('precacheArgs', () {
      group('when running on macOS', () {
        ...
        test('includes ios in platform list', () async {
          expect(
            runWithOverrides(() => quicuiFlutter.precacheArgs),
            contains('--ios'),
          );
        });
      });
    ...
    group('getConfig', () {
      late QuicuiProcessResult configProcessResult;
      ...
      group('when process exists with non-zero code', () {
        ...
        test('returns empty map', () {
          expect(runWithOverrides(quicuiFlutter.getConfig), isEmpty);
          verify(
...
All Settings:
  ...
  ''');
        ...
        test('returns correct config map', () {
          expect(
            runWithOverrides(quicuiFlutter.getConfig),
            equals({
              'enable-web': '(Not set)',
              'enable-linux-desktop': '(Not set)',
              'enable-macos-desktop': '(Not set)',
    ...
    group('getSystemVersion', () {
      test(
        ...
        () async {
          ...
          await expectLater(
            runWithOverrides(quicuiFlutter.getSystemVersion),
    ...
    group('getVersionAndRevision', () {
      group('when unable to determine version', () {
        ...
        test('returns unknown (<revision>)', () async {
          await expectLater(
            runWithOverrides(quicuiFlutter.getVersionAndRevision),
    ...
    group('resolveFlutterRevision', () {
        ...
        test(
          ...
          () async {
            final revision = await runWithOverrides(
              () => quicuiFlutter.resolveFlutterRevision('3.10.6'),
            );
      ...
      group('when input is a commit hash that maps to a Flutter version', () {
        ...
        test('returns the input string', () async {
          final revision = await runWithOverrides(
            () => quicuiFlutter.resolveFlutterRevision('deadbeef'),
          );
      ...
      group(
        ...
        () {
          ...
          test('returns the input string', () async {
            final revision = await runWithOverrides(
              () => quicuiFlutter.resolveFlutterRevision('not-a-version'),
            );
    ...
    group('resolveFlutterVersion', () {
      group('when input is a semver version', () {
        test(
          ...
          () async {
            final revision = await runWithOverrides(
              () => quicuiFlutter.resolveFlutterVersion('3.10.6'),
            );
      ...
      group('when input is not a recognized commit hash', () {
        ...
        test('returns null', () async {
          final revision = await runWithOverrides(
            () => quicuiFlutter.resolveFlutterVersion('not-a-version'),
          );
          expect(revision, isNull);
        });
      });
      ...
      group('when input is a recognized commit hash', () {
        ...
        test('returns a parsed version', () async {
          final revision = await runWithOverrides(
            () => quicuiFlutter.resolveFlutterVersion('deadbeef'),
          );
    ...
    group('getRevisionForVersion', () {
      ...
      group('when process exits with non-zero code', () {
        ...
        test('throws exception', () async {
          await expectLater(
            runWithOverrides(
              () => quicuiFlutter.getRevisionForVersion(version),
            ),
            throwsA(exception),
          );
          verify(
...
$revision
    ...
    group('getVersionString', () {
      group('when process exits with non-zero code', () {
        ...
        test('throws ProcessException', () async {
          await expectLater(
            runWithOverrides(quicuiFlutter.getVersionString),
            throwsA(isA<ProcessException>()),
          );
          verify(
            () => git.forEachRef(
    ...
    group('getVersion', () {
      group('when getVersionString returns null', () {
        ...
        test('returns null', () {
          expect(
            runWithOverrides(quicuiFlutter.getVersion),
            completion(isNull),
          );
        });
      });
...
origin/flutter_release/3.10.6''';
        ...
        await expectLater(
          runWithOverrides(quicuiFlutter.getVersions),
    ...
    group('installRevision', () {
      ...
      test('does nothing if the revision is already installed', () async {
        ...
        await runWithOverrides(
          () => quicuiFlutter.installRevision(revision: revision),
        );
      ...
      test('throws exception if unable to clone', () async {
        ...
        verify(
          () => git.clone(
            url: QuicuiFlutter.flutterGitUrl,
    ...
    group('isPorcelain', () {
      test('returns true when status is empty', () async {
        await expectLater(
          runWithOverrides(() => quicuiFlutter.isUnmodified()),
          completion(isTrue),
        );
        verify(
          () => git.status(

...
void main() {
  group(QuicuiVersion, () {
    ...
    late Git git;
    late QuicuiVersion quicuiVersionManager;
    ...
    setUp(() {
      git = MockGit();
      quicuiVersionManager = QuicuiVersion();
    ...
    group('isQuicuiVersionCurrent', () {
      test('returns true if current and latest git hashes match', () async {
        expect(
          await runWithOverrides(quicuiVersionManager.isLatest),
          isTrue,
        );
        verify(
          () => git.fetch(
      ...
      test('returns false if current and latest git hashes differ', () async {
        when(
          ...
          if (revision == 'HEAD') {
            return currentQuicuiRevision;
          } else if (revision == '@{upstream}') {
            return newerQuicuiRevision;
          }
      ...
      test(
        ...
        () async {
          ...
          expect(
            runWithOverrides(quicuiVersionManager.isLatest),
            throwsA(
    ...
    group('attemptReset', () {
      test('completes when git command exits with code 0', () async {
        expect(
          runWithOverrides(
            () => quicuiVersionManager.attemptReset(revision: 'HEAD'),
          ),
          completes,
        );
      });
    ...
    group('isTrackingStable', () {
      group('when on the stable branch', () {
        ...
        test('returns true', () async {
          expect(
            await runWithOverrides(quicuiVersionManager.isTrackingStable),
            isTrue,
          );
        });
      });

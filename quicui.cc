...
#include "flutter/shell/common/quicui/snapshots_data_handle.h"
...
// iOS-specific function to check for .vmcode patches using QuicUIPatchLoader.
// Returns a malloc'd string with the path if found, nullptr otherwise.
// The caller is responsible for freeing the returned string.
char* quicui_check_ios_vmcode_patch() {
  FML_LOG(INFO) << "[QuicUI] [DEBUG] quicui_check_ios_vmcode_patch() called";
  
  // Use QuicUIPatchLoader to get the patch path (it handles our patches/[id]/dlc.vmcode structure)
  std::string patch_path = flutter::QuicUIPatchLoader::GetPatchedAOTPath("arm64");
  
  FML_LOG(INFO) << "[QuicUI] [DEBUG] GetPatchedAOTPath returned: '" << patch_path << "' (length: " << patch_path.length() << ")";
  
  if (!patch_path.empty()) {
    // Verify it's a .vmcode file
    if (patch_path.find(".vmcode") != std::string::npos) {
      FML_LOG(INFO) << "[QuicUI] Found iOS .vmcode patch via QuicUIPatchLoader: " << patch_path;
      // Return a malloc'd copy that the caller must free
      return strdup(patch_path.c_str());
    } else {
      FML_LOG(INFO) << "[QuicUI] [DEBUG] Path doesn't contain .vmcode: " << patch_path;
    }
  }
  
  FML_LOG(INFO) << "[QuicUI] No iOS .vmcode patch found";
  return nullptr;
}
#endif
...
void SetBaseSnapshot(Settings& settings) {
  ...
  // Note: SetBaseSnapshots is not available in the current updater library version
  // This function may not be needed for the interpreter-based approach
  // QuicUI_SetBaseSnapshots(isolate_snapshot->GetDataMapping(),
...
FileCallbacks QuicUIFileCallbacks() {
  return {
...
// FIXME: consolidate this with the other ConfigureQuicUI
bool ConfigureQuicUI(const QuicUIConfigArgs& args,
                        std::string& patch_path) {
  patch_path = args.release_app_library_path;
  auto quicui_updater_dir_name = "quicui_updater";

  auto code_cache_dir = fml::paths::JoinPaths(
      {std::move(args.code_cache_path), quicui_updater_dir_name});
  auto app_storage_dir = fml::paths::JoinPaths(
      {std::move(args.app_storage_path), quicui_updater_dir_name});

  fml::CreateDirectory(fml::paths::GetCachesDirectory(),
                       {quicui_updater_dir_name},
  ...
  // Using a block to make AppParameters lifetime explicit.
  {
    ...
    // quicui_init copies from app_parameters and quicuiYaml.
    init_result = quicui_init(&app_parameters, QuicUIFileCallbacks(),
                                 args.quicui_yaml.c_str());
  }
  ...
  // https://github.com/quicuitech/quicui/issues/950

  FML_LOG(INFO) << "Checking for active patch";
  char* c_active_path = quicui_next_boot_patch_path();
  if (c_active_path != NULL) {
    patch_path = c_active_path;
    quicui_free_string(c_active_path);
    FML_LOG(INFO) << "QuicUI updater: patch path: " << patch_path;
  } else {
    FML_LOG(INFO) << "QuicUI updater: no active patch.";
  }

  // We are careful only to report a launch start in the case where it's the
  // first time we've configured quicui this process. Otherwise we could end
  ...
  // (and thus configures quicui) but never runs it. The proper fix for this
  ...
  // change their return values if the quicui_report_launch_failed
  // function is called.
  quicui_report_launch_start();

  if (quicui_should_auto_update()) {
    FML_LOG(INFO) << "Starting QuicUI auto update thread";
    quicui_start_update_thread();
  } else {
    FML_LOG(INFO)
        << "QuicUI auto_update disabled, not checking for updates.";
  }
...
void ConfigureQuicUI(std::string code_cache_path,
  ...
  FML_LOG(INFO) << "[QuicUI] [DEBUG] ConfigureQuicUI() called - START";
  
  // If you are crashing here, you probably are running QuicUI in a Debug
  ...
  FML_CHECK(DartSnapshot::VMSnapshotFromSettings(settings))
      << "XCode Scheme must be set to Release to use QuicUI";

  auto quicui_updater_dir_name = "quicui_updater";

  auto code_cache_dir = fml::paths::JoinPaths(
      {std::move(code_cache_path), quicui_updater_dir_name});
  auto app_storage_dir = fml::paths::JoinPaths(
      {std::move(app_storage_path), quicui_updater_dir_name});
  ...
  FML_LOG(INFO) << "[QuicUI] [DEBUG] About to call SetBaseSnapshot";
  SetBaseSnapshot(settings);
  FML_LOG(INFO) << "[QuicUI] [DEBUG] SetBaseSnapshot completed";
#endif

  FML_LOG(INFO) << "[QuicUI] [DEBUG] Before iOS vmcode patch check";
  
  // Check for iOS .vmcode patches first
#if defined(__APPLE__) && QUICUI_USE_INTERPRETER
  FML_LOG(INFO) << "[QuicUI] [DEBUG] Inside iOS vmcode block - calling quicui_check_ios_vmcode_patch()";
  char* ios_vmcode = quicui_check_ios_vmcode_patch();
  if (ios_vmcode != nullptr) {
    std::string active_path = ios_vmcode;
    free(ios_vmcode);
    FML_LOG(INFO) << "QuicUI: [iOS] Injecting vmcode path: " << active_path;
    
    // Add the patch to the front of the list to allow dart_snapshot.cc
    // to still find the base snapshot for the vm isolate.
    settings.application_library_paths.insert(
        settings.application_library_paths.begin(), active_path);
    
    // Don't continue with Android patching logic - iOS uses vmcode
    return;
  }
#endif
  ...
  if (c_active_path != NULL) {
    ...
    quicui_free_string(c_active_path);
    FML_LOG(INFO) << "QuicUI updater: active path: " << active_path;
  ...
  if (quicui_should_auto_update()) {
    FML_LOG(INFO) << "Starting QuicUI update";
    quicui_start_update_thread();
  } else {
    FML_LOG(INFO)
        << "QuicUI auto_update disabled, not checking for updates.";
